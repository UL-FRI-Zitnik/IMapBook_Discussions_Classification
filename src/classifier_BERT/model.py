...
class Bert_Model(Model):
    def __init__(self, target, english=False):
        ...
        self.model = bert_class.from_pretrained('classifier_BERT/pretrained_models/slo-hr-en-bert-pytorch',
                                                num_labels=self.n_class, from_pt=True)
    ...
    def fit(self, messages, y, epochs=2, validation_percent=0.15, allow_import=True):
        ...
        if os.path.exists(self.path) and allow_import:
            ...
            train_ds = tf.data.Dataset.from_tensor_slices(
                (X_train_input[0], X_train_input[1], X_train_input[2], y_train)).map(self.example_to_features).shuffle(
                100).batch(12).repeat(5)
            val_ds = tf.data.Dataset.from_tensor_slices((X_val_input[0], X_val_input[1], X_val_input[2], y_val)).map(
                self.example_to_features).batch(12)
    ...
    def predict_probabilities(self, messages):
        ...
        test_ds = tf.data.Dataset.from_tensor_slices(
            (X_test_input[0], X_test_input[1], X_test_input[2], np.ones(n))).map(self.example_to_features).batch(12)
